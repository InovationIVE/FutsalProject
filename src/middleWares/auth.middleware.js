...
const authMiddleware = async (req, res, next) => {
  const isSocket = !res || typeof res.status !== 'function';
  ...
  // 세션토큰이 없으면 인증 불가
  if (!sessionToken) {
    if (isSocket) return next(new Error('로그인이 필요합니다.'));
  ...
  try {
    ...
    if (negativeSessionCache.isNegative(sessionTokenHash)) {
      if (isSocket) return next(new Error('유효하지 않은 세션입니다 (캐시됨).'));
    ...
    if (cachedSession) {
      ...
      if (new Date(cachedSession.expiresAt) < new Date()) {
        ...
        if (isSocket) return next(new Error('세션이 만료되었습니다 (캐시 확인).'));
      ...
      if (!dbSession) {
        ...
        negativeSessionCache.setNegative(sessionTokenHash);
        if (isSocket) return next(new Error('세션이 존재하지 않습니다.'));
        ...
        userPrisma.session.delete({ where: { sessionTokenHash } }).catch();
        negativeSessionCache.setNegative(sessionTokenHash);
        if (isSocket) return next(new Error('세션이 만료되었습니다.'));
    ...
    if (isSocket) return next(new Error('서버 내부 오류가 발생했습니다.'));
