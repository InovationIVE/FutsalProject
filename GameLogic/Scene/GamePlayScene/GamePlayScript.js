document.addEventListener('DOMContentLoaded', () => {
        ...
        case 'shoot':
          ...
          if (selectedPlayer.shoot(opponentGoal, this.ball)) {
            ...
            const randomOpponent =
              this.opponentTeam.players[
                Math.floor(Math.random() * this.opponentTeam.players.length)
              ];
  ...
  async function initGame() {
    ...
    let players= await loadSquadTeam();

    const playerA1 = new Player(1, players[0].name, players[0].attack, players[0].defence, players[0].speed, 'teamA');
    playerA1.position = { x: 2, y: 2 };
    const playerA2 = new Player(2, players[1].name, players[1].attack, players[1].defence, players[1].speed, 'teamA');
    playerA2.position = { x: 1, y: 1 };
    const playerA3 = new Player(3, players[2].name, players[2].attack, players[2].defence, players[2].speed, 'teamA');
  ...
  function updateUI() {
    ...
    if (selectedPlayer) {
      ...
      const canShoot =
        game.ball.owner === selectedPlayer &&
        ((isTeamA && selectedPlayer.position.x > 3) || (!isTeamA && selectedPlayer.position.x < 3));

      const canGetBall =
        !game.ball.owner &&
        Math.abs(selectedPlayer.position.x - game.ball.position.x) +
          Math.abs(selectedPlayer.position.y - game.ball.position.y) <=
          1;
      ...
      tackleBtn.disabled =
        game.ball.owner === selectedPlayer ||
        !game.opponentTeam.players.some((p) => game.ball.owner === p);
...
async function loadSquadTeam() {
    // gachaPackList.innerHTML = '<p>스쿼드 목록을 불러오는 중...</p>';
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || '팀을 불러오는데 실패했습니다.');
      }
      const squadTeam = await response.json();
