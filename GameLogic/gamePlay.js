    ...
    if (this.currentTeam.players.includes(passer)) {
      if (!passer.pass(receiver, this.ball)) {
        ...
        const nearestOpponent = this.opponentTeam.players.find((p) => {
          const dist =
            Math.abs(p.position.x - passer.position.x) +
            Math.abs(p.position.y - passer.position.y);
          return dist <= 1;
        });
        if (nearestOpponent) {
...
function startGame(gameMap, goalA, goalB, ball, teamA, teamB) {
    ...
    while (currentTurn < maxTurn) {
        ...
        playCharater(
            game.currentTeam.players[choice - 1],
            game,
            gameMap,
            goalA,
            goalB
    ...
    switch (choice) {
        case '1': // 슛
            ...
            if (player.shoot(opponentGoal, game.ball)) {
                ...
                // 상대팀 선수 중 한명이 공을 가짐
                const randomOpponent = game.opponentTeam.players[Math.floor(Math.random() * game.opponentTeam.players.length)];
                ...
                game.ball.owner = null;
                // 골대 주변 랜덤 위치에 공 떨어짐
                const randomX = opponentGoal.position.x + (Math.random() < 0.5 ? -1 : 1) * Math.floor(Math.random() * 2 + 1);
                const randomY = opponentGoal.position.y + (Math.random() < 0.5 ? -1 : 1) * Math.floor(Math.random() * 2 + 1);
                ...
                for (const p of allPlayers) {
                    const dist = Math.abs(p.position.x - game.ball.position.x) + Math.abs(p.position.y - game.ball.position.y);
                    if (dist < minDistance) {
                        minDistance = dist;
                        nearestPlayer = p;
                    }
        ...
        case '4': // 태클
            ...
            if (opponentWithBall) {
                const tackleDistance = Math.abs(player.position.x - opponentWithBall.position.x) + Math.abs(player.position.y - opponentWithBall.position.y);
                if (tackleDistance <= 1) {
...
function mapView(gameMap, game, goalA, goalB) {
    ...
    for (const player of allPlayers) {
        if(player.position.y < 0 || player.position.y >= gameMap.height || player.position.x < 0 || player.position.x >= gameMap.width) continue;
